...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($result->num_rows === 1) {
        ...
        $updateQuery = "UPDATE users SET reset_token = ?, token_expiry = DATE_ADD(NOW(), INTERVAL 1 HOUR) WHERE email = ?";
        ...
        $resetLink = "http://localhost/furiture/resetpassword.php?token=$token";
        ...
        try {
            ...
            $mail->SMTPAuth = true;
            $mail->Username = 'athirabiju185@gmail.com'; // Your email
            $mail->Password = 'eesj okyk bnxf zttl'; // Your email password (use app password if 2FA is enabled)
